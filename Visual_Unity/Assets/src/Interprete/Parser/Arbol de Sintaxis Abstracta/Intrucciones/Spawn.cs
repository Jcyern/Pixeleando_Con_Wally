    ...
    public class SpawnNode : AstNode
    {
        ...
        public override bool CheckSemantic(ExpressionTypes tipo = ExpressionTypes.nothing)
        {
            if (fila != null && columna != null)
            {
                ...
                Debug.Log("Chequeo de Spawn");
                if (tipo_fila == tipo_columna && tipo_fila == ExpressionTypes.Number)
                {
                    Debug.Log("ambas expresiones son numericas");
                    return true;
                }
                if (tipo_fila != ExpressionTypes.Number)
                {
                    Debug.Log("La expression fila no devuelve un numero ");
                ...
                {
                    Debug.Log("La expression columna no devuelve un numero ");
                    //agregar error 
        ...
        public override object? Evaluate(Evaluator? evaluador = null)
        {
            ...
            var isf = f < Wally.canvas.Item1 && f >= 0;
            var isc = -c <= 0 && -c >-Wally.canvas.Item2;


            if (isf && isc)
            {
                
                Debug.Log($"se coloco al wally en la pos fila: {f}  columna: {-c}");
...
public class SpawnParser : IParse
{
    public AstNode Parse(Parser parser)
    {
        ...
        {
            Debug.Log($"Agregando izq a {parser.Current.value}");
        ...
        if (parser.Current.type == TypeToken.Coma)
        {
            ...
            {
                Debug.Log($"Agregando a der a {parser.Current.value}");
            ...
            Debug.Log("Retornar nodo de Spawn");
        ...
        else
        {
            ...
            Debug.Log("Retorna nodo de Spawn"); 
            return new SpawnNode(spawn, exp_izq, exp_der);
        }
